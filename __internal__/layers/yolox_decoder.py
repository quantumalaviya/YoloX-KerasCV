...
class DecodePredictions(keras.layers.Layer):
    ...
    def call(self, images, predictions):
        ...
        predictions = [tf.reshape(x, [batch_size, -1, 5 + self.classes]) for x in predictions]
        ...
        for i in range(len(shapes)):
            ...
            strides.append(tf.ones((shape[0], shape[1], 1)) * image_shape[0] / tf.cast(shape_x, tf.float32))

        grids = tf.concat(grids, axis = 1)
        strides = tf.concat(strides, axis = 1)

        box_xy = tf.expand_dims((predictions[..., :2] + grids) * strides / image_shape, axis = -2)
        box_xy = tf.broadcast_to(box_xy, [batch_size, predictions.shape[1], self.classes, 2])
        box_wh = tf.expand_dims(tf.exp(predictions[..., 2:4]) * strides / image_shape, axis = -2)
        box_wh = tf.broadcast_to(box_wh, [batch_size, predictions.shape[1], self.classes, 2])
        ...
        box_classes = tf.broadcast_to(box_classes, [batch_size, predictions.shape[1], self.classes, 1])
